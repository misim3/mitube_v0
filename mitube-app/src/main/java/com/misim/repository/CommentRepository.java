...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("SELECT c1 from Comment c1 WHERE c1.parentCommentId IS NULL AND c1.video.id = :videoId AND c1.id > :id ORDER BY c1.id LIMIT 11")
    List<Comment> findDownCommentByVideoIdAndId(@Param("videoId") Long videoId,
        @Param("id") Long id);

    @Query("SELECT c1 from Comment c1 WHERE c1.parentCommentId IS NULL AND c1.video.id = :videoId AND c1.id < :id ORDER BY c1.id DESC LIMIT 10")
    List<Comment> findUpCommentByVideoIdAndId(@Param("videoId") Long videoId, @Param("id") Long id);

    List<Comment> findCommentByVideoId(Long videoId);

    @Query("SELECT c1 from Comment c1 WHERE c1.parentCommentId = :parentCommentId AND c1.video.id = :videoId AND c1.id > :id ORDER BY c1.id LIMIT 11")
